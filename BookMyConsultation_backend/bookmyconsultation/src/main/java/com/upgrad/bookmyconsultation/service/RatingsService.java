...
public class RatingsService {
	...
	public void submitRatings(Rating rating) {
		...
		//using collector class functions to find average rating of doctor
		Double newRating = ratingsRepository.findByDoctorId(rating.getDoctorId()).stream().collect(Collectors.averagingInt(Rating::getRating));
		Doctor saveDoctor = doctorRepository.findById(rating.getDoctorId()).get();
		saveDoctor.setRating(newRating);
